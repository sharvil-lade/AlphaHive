...
class TextChunker:
    ...
    def segment_and_chunk(self, raw_text: str) -> list[dict[str, Any]]:
        ...
        # Locate the Item 1A (Risk Factors) and Item 7 (MD&A) headers.
        ...
        if item_1a_match and item_7_match:
            ...
            sections.append({"name": "General & Business Overview", "text": raw_text[:idx_1a]})

            # Risk Factors (between 1A and 7)
            sections.append({"name": "Item 1A. Risk Factors", "text": raw_text[idx_1a:idx_7]})
            ...
            if item_8_match and item_8_match.start() > idx_7:
                ...
                sections.append(
                    {"name": "Item 7. Management's Discussion and Analysis", "text": raw_text[idx_7:]}
                )
        else:
            # Fallback if specific item headers were not found
            sections.append({"name": "General Corporate Filing", "text": raw_text})
        ...
        for sec in sections:
            ...
            for text in split_texts:
                chunks.append({"text": text, "section": sec_name, "chunk_id": chunk_counter})
                chunk_counter += 1

        logger.info(
            f"Segmented filing into {len(sections)} sections, yielding {len(chunks)} total text chunks"
        )
        return chunks
