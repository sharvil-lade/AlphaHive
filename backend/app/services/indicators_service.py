...
class IndicatorsService:
    ...
    async def calculate_indicators(self, symbol: str) -> dict[str, Any]:
        ...
        # RSI 14, using Wilder's smoothing (com=13 is equivalent to a 14-period average).
        delta = df["close"].diff()
        ...
        macd_crossover_bullish = bool(
            prev_row["macd"] <= prev_row["macd_signal"] and latest_row["macd"] > latest_row["macd_signal"]
        )
        macd_crossover_bearish = bool(
            prev_row["macd"] >= prev_row["macd_signal"] and latest_row["macd"] < latest_row["macd_signal"]
        )
        ...
        return {
            ...
            if not pd.isna(latest_row["macd_signal"])
            else 0.0,
            ...
            if (latest_row["sma_200"] is not None and not pd.isna(latest_row["sma_200"]))
            else None,
            ...
            "volume_sma_20": float(latest_row["volume_sma_20"])
            if not pd.isna(latest_row["volume_sma_20"])
            else None,
