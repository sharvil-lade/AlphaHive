...
async def stream_agent_run_logs(run_id: UUID):
    """Stream real-time agent execution log entries via SSE."""
    return StreamingResponse(log_generator(run_id), media_type="text/event-stream")


@router.get("/run/{run_id}", response_model=AgentRunDetailResponse)
async def get_agent_run_detail(run_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not agent_run:
        raise HTTPException(
            ...
            detail=f"Agent run session not found for ID: {run_id}"
        )
