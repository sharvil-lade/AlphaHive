...
async def technical_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        if message_id:
            await emit_chat_event(message_id, {"type": "agent-status", "node": "technical", "status": "completed"})
        ...
        if message_id:
            await emit_chat_event(message_id, {"type": "agent-status", "node": "technical", "status": "failed"})
        return {
