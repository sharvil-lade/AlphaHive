...
async def research_node(state: AgentState) -> dict[str, Any]:
    ...
    start_log = await log_agent_activity(
        run_id, "research", f"Initializing fundamental research for ticker: {ticker}"
    )
    if message_id:
        await emit_chat_event(
            message_id, {"type": "agent-status", "node": "fundamentals", "status": "running"}
        )
    ...
    try:
        ...
        if message_id:
            await emit_chat_event(
                message_id, {"type": "agent-status", "node": "fundamentals", "status": "completed"}
            )

        return {"quotes": quotes_data, "logs": [start_log, success_log]}
    except Exception as e:
        error_log = await log_agent_activity(run_id, "research", f"Error during fundamental research: {e}")
        if message_id:
            await emit_chat_event(
                message_id, {"type": "agent-status", "node": "fundamentals", "status": "failed"}
            )
